...
test('verifyPin returns 400 on missing pin',   async () => {
  const res = await verifyPin(makeEvent({}), {});
  expect(res.statusCode).toBe(400);
});

test('verifyPin returns 400 on short pin', async () => {
  const res = await verifyPin(makeEvent({ pin: '123', inboxId: 'abc' }), {});
  expect(res.statusCode).toBe(400);
});

test('verifyPin returns 400 on non-numeric pin', async () => {
...
test('unlockInbox returns 400 on missing pin', async () => {
